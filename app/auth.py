...
def register_user(username: str, password_plain: str) -> tuple[str, str]:
    """
...
def authenticate_user(username: str, password_plain: str) -> tuple[int | None, str]:
    """
...
def recover_account(username: str, recovery_code_plain: str, new_password_plain: str) -> bool:
    """
    ...
    cursor = conn.cursor()
    try:
        cursor.execute("SELECT id, recovery_code_hash FROM users WHERE username = ?", (username,))
        row = cursor.fetchone()
        if not row:
            return False
...
def get_user_from_session(session_token: str) -> int | None:
    """
